...
class Unit:
    def __init__(self, dim, inputs, eta_v, pi_eta_x, pi_eta_v,
        ...
        self.grad_o_x = torch.zeros_like(self.x)
        self.grad_o_v = torch.zeros_like(self.v)
    ...
    def update(self, dt):
        ...
        dF_dx += self.grad_o_x + self.eps_eta_x
        dF_dx[0] += self.grad_x
        dF_dx[1] += self.eps_x

        dF_dv = self.eps_eta_v + self.grad_o_v + self.grad_v
...
class Obs:
    def __init__(self, dim, inputs, kind, pi_o, g, lr=None):
        # Point to inputs
        self.inputs = inputs
        self.kind = kind
    ...
    def predict(self, get_x=False):
        ...
        if self.kind == 'x':
            ...
            X = [inpt.v.clone().detach().requires_grad_(get_x)
                 for inpt in self.inputs]
    ...
    def step(self):
        ...
        for eta_o, inpt in zip(X, self.inputs):
            if self.kind == 'x':
                inpt.grad_o_x += eta_o.grad
            else:
                inpt.grad_o_v += eta_o.grad
