...
def record_video(log, width):
    ...
    def animate(n):
        ...
        for j in range(c.n_joints):
            # Draw estimated ball
            axs.plot(*np.array([est_ball_pos[n, idxs[j] + 1],
                                est_ball_pos[n, j + 1]]).T,
                     lw=size[j, 1] * scale, color='brown', zorder=1)

            # Draw estimated square
            axs.plot(*np.array([est_square_pos[n, idxs[j] + 1],
                                est_square_pos[n, j + 1]]).T,
                     lw=size[j, 1] * scale, color='g', zorder=1)
