...
class Window(pyglet.window.Window):
    def __init__(self):
        super().__init__(c.width, c.height, 'Intention modulation '
                         'and object affordances', vsync=False)
    ...
    def get_visual_obs(self):
        return utils.normalize([self.body.get_pos()[-1],
                                self.objects.ball.get_pos(),
                                self.objects.square.get_pos()], c.norm_cart)

    # Get tactile observation
    def get_tactile_obs(self):
        ball_pos = self.objects.ball.get_pos()
        hand_pos = self.body.get_pos()[-1]

        ball_reached = np.linalg.norm(ball_pos - hand_pos) < c.ball_size * 2

        return float(ball_reached)
