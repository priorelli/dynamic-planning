...
def kinematics(angles_norm, lengths, limits):
    ...
    # Wrist
    T_wrist = torch.eye(3)
    for j in range(len(angles) - 4):
        l = torch.tensor(lengths[j], dtype=torch.float32)
        c = torch.cos(torch.deg2rad(angles[j]))
        s = torch.sin(torch.deg2rad(angles[j]))

        T_rel = torch.stack([torch.stack([c, -s, l * c], -1),
                             torch.stack([s, c, l * s], -1),
                             torch.stack([z, z, i], -1)], -2)

        T_wrist = T_wrist.matmul(T_rel)
    ...
    for j in [-3, -4]:
        ...
        fingers.append(T_wrist.matmul(T_rel)[:2, 2])

    return (fingers[0] + fingers[1]) / 2
